...
def write_to_file(bin):
    with open("binary_count.txt","a") as myfile:
        myfile.write(bin+"\n")
...
if __name__ == '__main__':
    ...
    for k, v in fileData.items():
        ...
        try:
            ...
            methods1 = PatchReader(project, commitId1, commitId2).methods()
            patchName = commitId3 + commitId4 + ".patch"
            methods2 = PatchReader(project, commitId3, commitId4).methods()
            if len(methods1)==0 or len(methods2):

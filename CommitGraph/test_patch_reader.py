...
from CommitGraph.patch_reader import get_patch
...
class TestGetPatch(unittest.TestCase):
    ...
    def test_get_patch(self):
        patch = self.gp(self.target_repo, self.commit, self.previous_commit)

        assert isinstance(patch, str)

class TestPatchSplitter(unittest.TestCase):
    def setUp(self):
        self.ps = PatchSplitter(
                repo_name='hbase',
                commit=None,
                parent_commit=None)
...
class TestPatchSection(unittest.TestCase):
    def setUp(self):
        self.ps = PatchSection(
                repo_name='hbase',
                section=example_section.lstrip('diff --git'),
                commit=None,
                parent_commit=None)
